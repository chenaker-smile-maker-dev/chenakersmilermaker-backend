...
class ListTrainings
{
    ...
    {
        $trainings = Training::withCount(['approvedReviews'])->paginate($perPage, ['*'], 'page', $page);

        return [
            'data' => collect($trainings->items())->map(fn (Training $training) => $this->formatTraining($training))->values()->toArray(),
            'pagination' => [
