...
class BookingCalendarWidget extends CalendarWidget
{
    ...
    protected string $view = 'panels.widgets.calendar.booking-calendar-widget';
    ...
    // Filters (public for Livewire binding, arrays for multiple selection)
    /** @var array<int> */
    ...
    public string $activeView = CalendarViewType::DayGridMonth->value;

    public function getHeading(): string
    {
        return __('panels/admin/widgets/dashboard.calendar_heading');
    }
    ...
    protected function getEvents(FetchInfo $info): Collection|array|Builder
    {
        ...
        if (! empty($this->filterDoctorIds)) {
            $query->whereIn('doctor_id', $this->filterDoctorIds);
        }
        ...
        // ── Doctor schedules (blocked + availability) ─────────────────────────
        $scheduleQuery = \Zap\Models\Schedule::query()
            ->with('periods')
            ->where('schedulable_type', (new Doctor())->getMorphClass())
            ->whereIn('schedule_type', [ScheduleTypes::BLOCKED->value, ScheduleTypes::AVAILABILITY->value])
            ->where('is_active', true)
            ->where('start_date', '<=', $info->end)
            ->where(fn($q) => $q->whereNull('end_date')->orWhere('end_date', '>=', $info->start));

        if (! empty($this->filterDoctorIds)) {
            $scheduleQuery->whereIn('schedulable_id', $this->filterDoctorIds);
        }
            ...
            if ($isBlocked) {
                ...
                if ($isAllDay) {
                    $events[] = CalendarEvent::make()
                        ->title($schedule->name ?: __('panels/admin/widgets/dashboard.calendar_blocked'))
                        ->start($startDate)
                ...
                while ($current->lte($rangeEnd)) {
                    if (in_array(strtolower($current->format('l')), $days)) {
                        $events[] = CalendarEvent::make()
                            ->title($schedule->name ?: __('panels/admin/widgets/dashboard.calendar_availability'))
        ...
        return $schema->components([
            Section::make()
                ...
                ->schema([
                    ...
                    // Status transitions happen via dedicated context-menu actions.
                    Select::make('status')
    ...
    // ─── Date Click / Select – context menu ──────────────────────────────────
    ...
    // ─── Event Click – context menu (status-based) ───────────────────────────
    ...
    // ─── Filter helpers (Livewire lifecycle) ──────────────────────────────────

    public function updatedFilterDoctorIds(): void
    {
        $this->refreshRecords();
    }
    ...
    public function getDoctors(): \Illuminate\Support\Collection
    {
